...
def load_file(root, dataset):
    ...
    if dataset == "inaturelist2017":
        categories_2017 = [
            x["name"].strip().lower() for x in val_class_info["categories"]
        ]
...
def find_images_and_targets_cub200(root, dataset, istrain=False, aux_info=False):
    imageid2label = {}
    with open(
        os.path.join(os.path.join(root, "CUB_200_2011"), "image_class_labels.txt"), "r"
    ) as f:
        for line in f:
    ...
    imageid2split = {}
    with open(
        os.path.join(os.path.join(root, "CUB_200_2011"), "train_test_split.txt"), "r"
    ) as f:
        for line in f:
    ...
    with open(os.path.join(os.path.join(root, "CUB_200_2011"), "images.txt"), "r") as f:
        for line in f:
            ...
            if aux_info:
                with open(
                    os.path.join(
                        bert_embedding_root, file_name.replace(".jpg", ".pickle")
                    ),
                    "rb",
                ) as f_bert:
                ...
                text_list = []
                with open(
                    os.path.join(text_root, file_name.replace(".jpg", ".txt")), "r"
                ) as f_text:
                    for line in f_text:
...
def find_images_and_targets_cub200_attribute(
    root, dataset, istrain=False, aux_info=False
):
    imageid2label = {}
    with open(
        os.path.join(os.path.join(root, "CUB_200_2011"), "image_class_labels.txt"), "r"
            ...
            if istrain and imageid2split[int(image_id)] == 1:
                if aux_info:
                    images_and_targets.append(
                        [file_path, target, imageid2attribute[int(image_id)]]
                    )
...
def find_images_and_targets_oxfordflower(root, dataset, istrain=False, aux_info=False):
    ...
    for data in all_data:
        ...
        if aux_info:
            with open(
                os.path.join(bert_embedding_root, f"image_{str(data).zfill(5)}.pickle"),
                "rb",
            ) as f_bert:
                bert_embedding = pickle.load(f_bert)
...
def find_images_and_targets_nabirds(root, dataset, istrain=False, aux_info=False):
    ...
    image_paths = pd.read_csv(
        os.path.join(root, "images.txt"), sep=" ", names=["img_id", "filepath"]
    )
...
def find_images_and_targets_stanfordcars_v1(
    root, dataset, istrain=False, aux_info=False
):
    if istrain:
        flag = "train"
        ...
        flag = "test"
    if istrain:
        anno_data = scio.loadmat(
            os.path.join(os.path.join(root, "devkit"), f"cars_{flag}_annos.mat")
        )
    else:
        anno_data = scio.loadmat(
            os.path.join(
                os.path.join(root, "devkit"), f"cars_{flag}_annos_withlabels.mat"
            )
        )
...
class DatasetMeta(data.Dataset):
    def __init__(
        ...
        if dataset in ["inaturelist2021", "inaturelist2021_mini"]:
            images, class_to_idx, images_info = find_images_and_targets(
                root, train, aux_info
            )
        elif dataset in ["inaturelist2017", "inaturelist2018"]:
            images, class_to_idx, images_info = find_images_and_targets_2017_2018(
                root, dataset, train, aux_info
            )
        elif dataset == "cub-200":
            images, class_to_idx, images_info = find_images_and_targets_cub200(
                root, dataset, train, aux_info
            )
        elif dataset == "stanfordcars":
            images, class_to_idx, images_info = find_images_and_targets_stanfordcars(
                root, dataset, train
            )
        elif dataset == "oxfordflower":
            images, class_to_idx, images_info = find_images_and_targets_oxfordflower(
                root, dataset, train, aux_info
            )
        elif dataset == "stanforddogs":
            images, class_to_idx, images_info = find_images_and_targets_stanforddogs(
                root, dataset, train
            )
        elif dataset == "nabirds":
            images, class_to_idx, images_info = find_images_and_targets_nabirds(
                root, dataset, train
            )
        elif dataset == "aircraft":
            images, class_to_idx, images_info = find_images_and_targets_aircraft(
                root, dataset, train
            )
        if len(images) == 0:
            raise RuntimeError(
